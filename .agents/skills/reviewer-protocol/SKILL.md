...
### Multi-Reviewer Gate (Architect + Code Reviewer + Security Reviewer)

Use this gate before merge for feature PRs:

1. Request an **Architect review** focused on architecture fit, boundaries, tenancy/isolation, lifecycle, and scalability tradeoffs.
2. Request a **Code Reviewer review** focused on correctness defects, race conditions, API misuse, validation gaps, and test reliability.
3. Request a **Security Reviewer review** focused on authz/authn gaps, cross-tenant leakage, abuse vectors, and sensitive data exposure.
4. Capture three explicit verdicts: `APPROVE` or `CHANGES REQUESTED`.
5. Merge is allowed only when **all three** reviewers approve.
6. If any reviewer rejects, apply lockout/reassignment rules and rerun all required reviews after revision.
7. Publish reviewer outcomes to GitHub PR comments so findings are visible in-thread.

Recommended review checklist for all passes:
- Scope alignment with issue/PBI acceptance criteria
- Safety constraints (tenant isolation, auth/validation, error handling)
- Runtime reliability (disconnect/retry/lifecycle behavior)
- Test coverage of critical paths and regressions

### Publishing Reviewer Findings to GitHub

After each review batch:

1. Post an **Architect Review** comment on the PR with verdict and blocking issues.
2. Post a **Code Review** comment on the PR with verdict and blocking issues.
3. Post a **Security Review** comment on the PR with verdict and blocking issues.
4. If all approve, post a merge-ready summary; if any reject, post a consolidated action list.
5. Keep comments structured and traceable to file paths and concrete fixes.
